...
const ProjectCard = ({ project }: any) => (
    <div className="group bg-card border border-border hover:border-brand-blue/30 hover:bg-card-hover p-6 rounded-xl transition-all duration-300 hover:-translate-y-1 shadow-sm dark:shadow-none">
        ...
        <p className="text-secondary text-sm leading-relaxed mb-6 border-b border-border pb-6">
            {project.description}
        </p>
        <div>
            <span className="text-brand-purple text-xs uppercase tracking-wide font-semibold block mb-2">Impact</span>
            <p className="text-primary text-sm">{project.impact}</p>
        </div>
    </div>
);
